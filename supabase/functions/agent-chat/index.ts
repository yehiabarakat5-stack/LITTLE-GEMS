...
async function getTodaySnapshot(svc: SupabaseClient, today: string): Promise<string> {
  ...
  const snapshot = `=== TODAY AT LITTLE GEMS (${today}) ===
...
async function buildSystemPrompt(
  ...
  const prompt = `You are an AI assistant for Little Gems by My Second Home, a premium pet
...
or claim you do not have access. You have full access to the Little Gems
...
const MSH_TOOLS: Anthropic.Tool[] = [
  {
    name: "query_database",
    description: `Query any Little Gems Supabase table. Supports PostgREST
